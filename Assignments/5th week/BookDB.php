...
class BookDB {
    ...
    public static function getAllBooks() {
        ...
        $books[2] = new Book(2, "Arhitektura računalniških sistemov", "Dušan Kodek", 20);
        $books[3] = new Book(3, "Managing Information Systems Security and Privacy", "Denis Trček", 15);
        $books[4] = new Book(4, "Študijski koledar", "FRI", 23.5);
    ...
    public static function find($query) {
        ...
        $all_books = self::getAllBooks();

        $results = array();

        foreach ($all_books as $book) {
            if (stripos($book->getTitle(), $query) !== false) {
                $results[] = $book; //apprend result
            }
        }
