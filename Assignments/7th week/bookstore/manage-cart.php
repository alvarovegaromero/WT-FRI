...
$quantity_edition = (isset($_POST["quantity_edition"])) ? intval($_POST["quantity_edition"]) : -1;
...
switch ($cart_action) {
    ...
    case 'edit': //only appear if session exist - so conditional could be removed

        //print($quantity_edition);

        if($quantity_edition == 0)
            unset($_SESSION["cart"][$id]);
        else
            $_SESSION["cart"][$id] = $quantity_edition;
